<script lang="ts">
	...
	import type { Fixture, Player, AvailabilityMap, FinalSelectionsMap } from '$lib/api/availability';
	...
	let availableCount = $derived(players.filter(p => isAvailable(p.id)).length);
	
	// Validation state
	let isValid = $derived(finalSelections.length === 3);
	let hasWarning = $derived(finalSelections.length > 0 && finalSelections.length !== 3);
</script>
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
	...
	<div class="h-48 bg-gradient-to-br from-blue-500 to-emerald-500 relative overflow-hidden">
	...
	<div class="p-6">
		...
		<div class="space-y-2 mb-4">
			...
			{#each players as player}
				<label class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/50 cursor-pointer transition-colors">
					<input
						type="checkbox"
						checked={isAvailable(player.id)}
						onchange={(e) => onAvailabilityChange(player.id, e.currentTarget.checked)}
		...
		<div class="border-t border-gray-200 dark:border-slate-700 pt-4">
			<div class="flex items-center justify-between mb-2">
				...
				<span class="text-xs px-2 py-1 rounded-full {isValid ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : hasWarning ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400'}">
				...
				{#each players.filter(p => isAvailable(p.id)) as player}
					<label class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700/50 cursor-pointer transition-colors">
						<input
							...
							disabled={!isSelected(player.id) && finalSelections.length >= 3}
			...
			{#if hasWarning}
				<div class="mt-3 p-2 rounded-lg bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-900/20">
					<p class="text-xs text-yellow-700 dark:text-yellow-500">
						{#if finalSelections.length < 3}
							⚠️ Please select {3 - finalSelections.length} more player{3 - finalSelections.length > 1 ? 's' : ''}
						{:else}
